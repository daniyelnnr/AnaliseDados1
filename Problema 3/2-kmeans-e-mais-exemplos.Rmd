...
theme_set(theme_bw())
source("https://raw.githubusercontent.com/nazareno/ciencia-de-dados-1/master/3-Agrupamento-e-PCA/github-lib.R")
```

```{r}
load_github_wide <- function(){
  d <- read.csv("https://raw.githubusercontent.com/nazareno/ciencia-de-dados-1/master/3-Agrupamento-e-PCA/dados/atividade-no-github-archive-2012-2015.csv", stringsAsFactors = TRUE)
...
dw2 <- dw2 %>% 
    mutate(cluster = hc %>% 
               cutree(k = n_clusters) %>% 
               as.character())
# cutree(hc, k = n_clusters) %>% as.character()
